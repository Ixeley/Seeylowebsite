...
export function AnalysisResult({ analysis: a, tradeStyle, onReset, onReanalyze }: Props) {
  ...
  const hasLevels =
    (a.supportLevels?.length ?? 0) > 0 ||
    (a.resistanceLevels?.length ?? 0) > 0 ||
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-muted-foreground mb-1">
            <Sparkles className="h-3.5 w-3.5 text-primary" /> GPT-4o Analysis
          </div>
        ...
        <div>
          ...
          >
            <span className="font-medium flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">({(a.keyLevels?.length ?? 0) + (a.supportLevels?.length ?? 0) + (a.resistanceLevels?.length ?? 0)})</span>
            </span>
          ...
          {levelsOpen && (
            <div className="mt-2 space-y-1.5 animate-fade-up">
              ...
              {a.supportLevels?.map((p, i) => <SimpleLevel key={`s${i}`} color="blue" label="Support" price={p} />)}
              {a.resistanceLevels?.map((p, i) => <SimpleLevel key={`r${i}`} color="amber" label="Resistance" price={p} />)}
            </div>
          )}
        </div>
      )}
...
function PriceRow({ icon, label, value, accent, highlight }: {
  ...
  accent?: "bullish" | "bearish"; highlight?: boolean;
}) {
...
function SimpleLevel({ color, label, price }: { color: "blue" | "amber"; label: string; price: number }) {
  ...
  return (
    <div className={`flex items-center justify-between rounded-lg border px-4 py-2 text-sm ${meta.cls}`}>
      <div className="flex items-center gap-2">
        <span className="font-bold text-xs min-w-[40px]">{meta.label}</span>
        <span className="text-muted-foreground">{kl.description}</span>
      </div>
      <span className="font-mono font-semibold">{fmt(kl.price)}</span>
    </div>
  );
}
